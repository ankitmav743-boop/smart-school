...
export function TeacherDashboard() {
  ...
  // Subject comes from teacher session (set at login)
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-blue-400 tracking-wider uppercase flex items-center gap-1.5">
        ...
        <AnimatePresence mode="wait">
          ...
          {!selectedClass ? (
            <motion.div key="class-selection" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              {/* Subject banner at top */}
              <div className="mb-6 inline-flex items-center gap-2 bg-blue-600 text-white px-5 py-2.5 rounded-2xl shadow-lg shadow-blue-500/20">
                <FlaskConical className="w-4 h-4" />
                <span className="font-bold text-sm">Subject: {selectedSubject}</span>
              </div>
              <div className="mb-8">
                <h2 className="text-3xl font-black text-slate-800 mb-2">Class Directory</h2>
                <p className="text-slate-500 font-medium">Select a class to manage <span className="font-bold text-blue-600">{selectedSubject}</span> records.</p>
              </div>
            ...
            <motion.div key="dashboard" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4">
                  <button onClick={() => { setSelectedClass(''); setStudents([]); setMarks([]); }} className="bg-white p-3 rounded-xl shadow-sm border border-slate-200 hover:text-blue-600 transition-colors">
                    <ArrowLeft className="w-5 h-5" />
                  </button>
                  <div>
                    <h2 className="text-3xl font-black text-slate-800">Class {selectedClass}</h2>
              ...
              <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-6 mb-8 relative overflow-hidden">
                ...
                <p className="text-sm text-slate-400 font-medium mb-5 relative z-10">
                  Subject: <span className="font-bold text-slate-600">{selectedSubject}</span>
                </p>
                <form onSubmit={handleAddHomework} className="flex flex-col md:flex-row gap-4 relative z-10">
                  <input
                    type="text"
                    placeholder={`Homework details for ${selectedSubject}...`}
                    value={homeworkDescription}
                    onChange={(e) => setHomeworkDescription(e.target.value)}
                    className="flex-1 px-5 py-3 border-2 border-slate-100 rounded-xl focus:ring-4 focus:ring-blue-100 focus:border-blue-500 outline-none font-medium bg-slate-50"
                    required
                  />
                  <input
                    type="date"
                    ...
                    className="px-5 py-3 border-2 border-slate-100 rounded-xl focus:ring-4 focus:ring-blue-100 focus:border-blue-500 outline-none font-medium bg-slate-50"
                    required
                  />
                    ...
                    return (
                      <motion.div variants={cardVariants} key={student.id} className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm hover:shadow-xl transition-shadow flex flex-col">
                        ...
                        <div className="grid grid-cols-2 gap-3 mt-auto">
                          <button
                            onClick={() => { setActiveStudentId(student.id); setModalType('Marks'); setMarkValue(''); setTotalMarks('100'); }}
                            className="flex items-center justify-center gap-2 bg-indigo-50 hover:bg-indigo-600 hover:text-white text-indigo-700 font-bold py-3 rounded-xl transition-colors"
                          >
                            <BookOpen className="w-4 h-4" /> Exam Mark
                          </button>
                          <button
                            onClick={() => { setActiveStudentId(student.id); setModalType('Weekly'); setMarkValue(''); }}
                            className="flex items-center justify-center gap-2 bg-emerald-50 hover:bg-emerald-600 hover:text-white text-emerald-700 font-bold py-3 rounded-xl transition-colors"
                          >
          ...
          {activeStudentId && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
              <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-white rounded-[2rem] p-8 w-full max-w-md shadow-2xl">
                ...
                <div className="inline-flex items-center gap-1.5 bg-blue-50 text-blue-700 text-xs font-bold px-3 py-1 rounded-full mb-6 border border-blue-100">
